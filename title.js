...
function drawRocketBlimp(L, t) {
  ...
  // the searchlight: from the gondola down to a pool of light on the water, swinging
  const gx = x - 2, gy = y + 9, tx = Math.round(x - 14 + Math.sin(t * .8) * 34), ty = Math.round(L.shoreY - 3), rows = ty - gy;
  if (rows > 4) { ctx.globalAlpha = .07; for (let r = 0; r < rows; r += 1) { const f = r / rows, cx = gx + (tx - gx) * f, half = 1 + f * 13; rect(Math.round(cx - half), gy + r, Math.round(half * 2), 1, '#fff4c0'); } ctx.globalAlpha = .2; ellipse(tx, ty, 15, 3, '#fff4c0'); ctx.globalAlpha = .12; ellipse(tx, ty, 22, 4, '#fff4c0'); ctx.globalAlpha = 1; }
  // the hull: a dark capsule with its fins aft (it flies left), a red band carrying the R, a lit gondola and a blinking light
...
function optionsErase(yes) {
  ...
  if (!PARAMS.has('nosave')) try { const keys = ['pk_save', 'pk_suspend', 'pk_records', 'pk_mute'].concat(Object.keys(PREF).map(k => 'pk_' + k)); for (let i = 0; i < (localStorage.length || 0); i++) { const k = localStorage.key(i); if (k && k.startsWith('pk_') && !keys.includes(k)) keys.push(k); } for (const k of keys) localStorage.removeItem(k); } catch (_) { }
