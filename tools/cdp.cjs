...
async function main() {
  ...
  try {
    ...
    if (script === 'flow' || script === 'full') {
      ...
      await key('z', 'KeyZ'); await waitScene('route'); await sleep(3500); await shot('19-route-reveal'); out.push('route after win: sel=' + await ev('__pk.SC.data.sel') + ' chapter=' + await ev('__pk.SAVE.chapter'));
    }
    ...
    if (script === 'mech') {
      ...
      await ev('(function(){const t=__pk.B.units.find(u=>u.name==="Caterpie"); t.hp=2; t.x=4; t.y=8; const s=__pk.B.units.find(u=>u.name==="Squirtle"); s.x=5; s.y=8; __pk.BT.cx=5; __pk.BT.cy=8;})()');
      ...
      // seize on ch4
      await nav('ch=4&silent&nosave'); await waitMode('idle', 6000); await ev('(function(){const u=__pk.B.units.find(u=>u.team===0); u.x=17; u.y=4; __pk.BT.cx=17; __pk.BT.cy=4;})()'); await key('z','KeyZ'); await sleep(200); await key('z','KeyZ'); await waitMode('menu', 4000); out.push('seize menu: ' + await ev('JSON.stringify(__pk.BT.menu.items.map(i=>i.id))')); await key('z','KeyZ'); await sleep(2000); await shot('37-seized'); out.push('ch4 result: ' + await ev('__pk.B.result'));
    ...
    if (script === 'ui2' || script === 'ui2-m') {
      ...
      await ev('__pk.SC.hits.find(h=>h.label==="BATTLE! ▸").run()'); await waitScene('card', 5000); await sleep(700); await shot('ui2-card');
    ...
    if (script === 'page' || script === 'page-m') {
      ...
      await send('Page.navigate', { url: process.env.PK_URL || 'file://' + ROOT + '/index.html?' + (process.env.PK_Q || 'silent&nosave') + LQ }); await sleep(+(process.env.PK_WAIT || 1500)); // PK_URL: any page, e.g. the published build
