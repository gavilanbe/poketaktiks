...
function versusDraw() {
  ...
  const d = DEX[VS_ROSTER[SC.i]]; if (d) { const iy = gy + rows * chh + 5; const u = makeUnit(d.num, S.level, 0); text(d.name, gx, iy, UI.ink, { outline: UI.shadow }); d.types.forEach((tp, j) => typeBadge(tp, gx + textWidth(d.name) + 6 + j * 26, iy - 1, 24)); const mv = u.moves.slice(0, 3).map(m => m.name).join(' / '); if (narrow) text(mv, gx, iy + 10, '#98d8f8', { outline: UI.shadow }); else { textR(mv, gx + cols * cw, iy, '#98d8f8', { outline: UI.shadow }); text('HP ' + u.maxHp + '  ATK ' + u.atk + '  DEF ' + u.def + '  SPA ' + u.spa + '  SPE ' + u.spe + '  MOV ' + u.mov, gx, iy + 10, UI.muted, { outline: UI.shadow }); } }
  // settings and the bottom row: one row each on wide screens, two rows each on phones
  const sy = narrow ? H - 2 * (bh + 4) - sbh - 8 : H - 44; const setting = (x, label, val, dec, inc) => { text(label, x, sy + (sbh - 7) / 2, UI.muted, { outline: UI.shadow }); bigButton(x + 34, sy, 18, sbh, '-', dec, { small: true }); textC(String(val), x + 64, sy + (sbh - 7) / 2, UI.gold, { outline: UI.shadow }); bigButton(x + 76, sy, 18, sbh, '+', inc, { small: true }); };
  setting(8, 'Arena', S.seed, () => { S.seed = (S.seed + 999) % 1000; Audio.sfx('menu'); }, () => { S.seed = (S.seed + 1) % 1000; Audio.sfx('menu'); });
  setting(narrow ? 100 : 112, 'Level', S.level, () => { S.level = Math.max(5, S.level - 5); Audio.sfx('menu'); }, () => { S.level = Math.min(50, S.level + 5); Audio.sfx('menu'); });
...
function starterDraw() {
  ...
  if (W < cw * 3 + gap * 2 + 8) { // phones: three wide, short cards stacked down the screen
    const w = W - 12, h = Math.min(56, Math.floor((H - 70) / 3) - 6);
    STARTERS.forEach((n, i) => {
      const d = DEX[n]; const x = 6, y = 44 + i * (h + 6); const sel = SC.i === i; panel(x, y, w, h, { fill: sel ? '#2a3d6a' : UI.panel, border: sel ? UI.gold : UI.border });
      const bob = sel ? Math.round(Math.abs(Math.sin(SC.t * 6)) * -3) : 0; rect(x + 6, y + 6, 40, h - 12, teamColorD(0)); drawMon(n, x + 26, y + h - 8 + bob, {});
      text(d.name, x + 52, y + 6, sel ? UI.gold : UI.ink); d.types.forEach((t, j) => typeBadge(t, x + 52 + textWidth(d.name) + 6 + j * 26, y + 5, 24));
      const u = makeUnit(n, 5, 0, { hpBonus: BOND_HP }); text('HP ' + u.maxHp + ' ATK ' + u.atk + ' DEF ' + u.def + ' SPA ' + u.spa + ' SPE ' + u.spe, x + 52, y + 17, UI.muted);
      let mv = 'MOV ' + u.mov + ' · ' + u.moves.map(m => m.name).join('/'); while (textWidth(mv) > w - 58 && mv.length > 6) mv = mv.slice(0, -1); text(mv, x + 52, y + 28, UI.muted);
      if (h >= 48) text(sel ? (VIEW.touch ? 'tap again to choose' : 'Z to choose') : '', x + 52, y + 39, UI.gold);
