...
function menuCard(x, y, w, h, label, sub, icon, hot, run, opt = {}) {
  ...
  if (sub) { let s = sub; const avail = w - iw - 18 - (opt.mons && opt.mons.length && w >= 150 ? Math.min(4, opt.mons.length) * 15 + 6 : 0); while (textWidth(s) > avail && s.length > 4) s = s.slice(0, -1); if (s !== sub) s = s.replace(/[ ·]+$/, '') + '…'; text(s, tx, y + h / 2 + 2, hot ? '#ffe9a0' : UI.muted); }
  if (opt.mons && opt.mons.length && w >= 150) { const n = Math.min(4, opt.mons.length); for (let i = 0; i < n; i++) { const mx = x + w - 14 - (n - i) * 15; ctx.globalAlpha = hot ? 1 : .75; ctx.drawImage(monIcon(opt.mons[i]), mx, y + h / 2 - 8 + (hot ? Math.round(Math.sin(SC.t * 6 + i) * 1) : 0), 20, 15); ctx.globalAlpha = 1; } }
...
function titleDraw() {
  ...
  if (save) items.push(['CONTINUE', save.beaten ? 'Campaign complete · replay' : 'Chapter ' + (Math.min(save.chapter, CHAPTERS.length - 1) + 1) + ' · ' + CHAPTERS[Math.min(save.chapter, CHAPTERS.length - 1)].title, 'flag', () => continueCampaign(), (save.party || []).map(p => p.num)]);
  ...
  items.forEach((it, i) => { const xx = cols === 2 ? W / 2 - bw - 4 + (i % 2) * (bw + 8) : bx; const yy = cols === 2 ? firstY + Math.floor(i / 2) * (bh + gap) : by; menuCard(xx, yy, bw, bh, it[0], it[1], it[2], SC.i === i, () => { Audio.sfx('ok'); it[3](); }, { mons: it[4] }); by += bh + gap; });
...
function prepDraw() {
  ...
  if (rx > gx + 200) { const py = ry + 122, avail = H - foot - py - 4; const bd = P.bd; const sc = Math.min((rw - 12) / bd.canvas.width, (avail - 27) / bd.canvas.height);
    if (sc >= .22) { const pw2 = Math.round(bd.canvas.width * sc), ph2 = Math.round(bd.canvas.height * sc); const foes = (ch.map.units || []).filter(u => u.team == null || u.team === 1), wild = (ch.map.units || []).filter(u => u.team === 2); const lv = foes.map(u => u.level);
      const p = panel(rx, py, rw, ph2 + 27, { header: 'BATTLEFIELD', headerRight: foes.length + ' foe' + (foes.length === 1 ? '' : 's') + (lv.length ? ' · Lv ' + Math.min(...lv) + (Math.max(...lv) > Math.min(...lv) ? '-' + Math.max(...lv) : '') : '') + (wild.length ? ' · ' + wild.length + ' wild' : '') });
