...
function journeyDraw() {
  ...
  if (pw && oak) { // Oak on a band of his colour, sliding in from the left with light running across it
    const px0 = x + 10, py0 = p.cy + 2, ph = Math.min(70, h - (p.cy - y) - 36), col = '#c8a060', slide = REDUCED ? 0 : Math.round((1 - easeOut(clamp(t / .35, 0, 1))) * -30);
    for (let j = 0; j < ph; j++) rect(px0, py0 + j, pw, 1, mix(shade(col, -.3), shade(col, -.78), j / ph)); outline(px0 - 1, py0 - 1, pw + 2, ph + 2, UI.border2);
    ctx.save(); ctx.beginPath(); ctx.rect(px0, py0, pw, ph); ctx.clip(); if (!REDUCED) { ctx.globalAlpha = .14; for (let q = 0; q < 5; q++) { const ly = py0 + 3 + (q * 13) % Math.max(4, ph - 6), len = 12 + (q * 9) % 20, lx = px0 + ((t * (50 + q * 12) + q * 23) % (pw + len)) - len; rect(Math.round(lx), ly, len, 1, '#ffffff'); } ctx.globalAlpha = 1; }
    ctx.drawImage(oak, px0 + Math.round((pw - 80) / 2) + slide, py0 + ph - 72 + (REDUCED ? 0 : Math.round(Math.abs(Math.sin(t * 3)))), 80, 80); ctx.restore(); }
  ...
  unfoldEnd(tok);
  const ready = REDUCED || t > .25 + rows.length * .18;
  mkCTA(x + 8, y + h - 26, w - 16, 20, 'LET\'S GO', () => { Audio.sfx('ok'); S.next(); }, { variant: 'primary', hot: ready, glow: '#8ae89a' });
}
