...
function routeDraw() {
  ...
  for (let i = 0; i < CHAPTERS.length; i++) {
    ...
    // who holds it: a Rocket pennant over a front still held, the freed Gym Leader's face over a cleared one
    { const ch = CHAPTERS[i], cleared = SAVE && SAVE.chapter > i, freed = cleared && ch.co && CO_UNLOCK[ch.co] && COS[ch.co], fx = x + rx + 1, fy = y - 17;
      if (freed) { const face = trainerFace(freed.tr); if (face) { rect(fx - 1, fy - 1, 12, 12, freed.col); ctx.save(); ctx.beginPath(); ctx.rect(fx, fy, 10, 10); ctx.clip(); ctx.drawImage(face, fx - 4, fy - 3); ctx.restore(); } }
      else if (!cleared && (ch.co || ch.foe)) { const wave = REDUCED ? 0 : Math.round(Math.sin(t * 4 + i)); vline(fx + 1, fy - 1, 12, '#3a2a20'); rect(fx + 2, fy + wave * .5, 7, 5, '#d23030'); px(fx + 4, fy + 1 + wave * .5, '#ffffff'); px(fx + 5, fy + 2 + wave * .5, '#ffffff'); px(fx + 4, fy + 3 + wave * .5, '#ffffff'); } }
